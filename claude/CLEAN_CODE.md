...
Apply the following clean code rules to ALL code you write, review, or refactor.
...
7. Command-query separation — a function either changes state or returns information, never
   both. `if (set("username", "unclebob"))` is ambiguous. Separate it into
   `attributeExists()` and `setAttribute()`.
...
2. Do not comment bad code — rewrite it. Spending time on comments that explain a mess is time
   better spent cleaning the mess.
3. Delete commented-out code — source control remembers it. Commented-out code accumulates and
   nobody has the courage to delete it.
...
5. Avoid: redundant comments that restate the code, journal comments (use source control),
   noise comments (`/** The day of the month. */ private int dayOfMonth;`), closing brace
   comments, attribution bylines, mandated javadocs for every function, and HTML in comments.
...
<formatting_rules>
1. Keep source files small — aim for 200 lines, upper limit around 500. Small files are easier
   to understand.
...
6. Keep lines under 120 characters. Shorter lines are easier to read.
7. Use horizontal whitespace to associate related things and separate unrelated things — space
   around assignment operators, no space between function name and parenthesis.
8. Never break indentation — even for short if statements or while loops. Always expand and
   indent scopes.
...
3. Favor many small, focused classes over a few large ones — the total complexity is the same,
   but small classes are individually comprehensible and independently testable.
...
5. Organize for change — classes should be open for extension but closed for modification
   (Open/Closed Principle). Use polymorphism and inheritance to add behavior without modifying
   existing code.
6. Depend on abstractions, not concrete details (Dependency Inversion Principle) — define
   interfaces for volatile dependencies and inject them via constructors.
...
1. Use exceptions, not return codes — exceptions separate the happy path from error handling
   and produce cleaner calling code.
2. Write try-catch-finally first — this defines the scope and establishes what the caller can
   expect regardless of what goes wrong.
3. Provide context with exceptions — include the operation that failed and the type of failure
   so the error can be logged and diagnosed.
...
5. Use the Special Case Pattern — instead of returning null or throwing exceptions for expected
   edge cases, return a special case object that encapsulates the default behavior.
...
1. Encapsulate third-party interfaces — do not pass boundary types like `Map` throughout your
   system. Wrap them in application-specific classes that enforce your design rules.
2. Write learning tests for third-party code — small tests that verify your understanding of
   the API. They cost nothing (you had to learn the API anyway) and detect behavioral changes
   in new releases.
...
4. Keep third-party references confined to as few places as possible — wrap or adapt them so
   your code depends on something you control.
</boundary_rules>

<testing_rules>
1. Test code is as important as production code — keep it clean, readable, and well-maintained.
   Dirty tests are worse than no tests because they become a liability that the team eventually
   discards.
...
5. Tests must be FIRST: Fast, Independent, Repeatable, Self-Validating, and Timely (written
   just before the production code).
6. Build a domain-specific testing language — helper functions and utilities that make tests
...
2. Separate construction from use — the startup process (object creation and wiring) should be
   distinct from runtime logic. Use dependency injection.
...
8. Keep configurable data at high levels of abstraction — defaults and configuration values
   should not be buried in low-level functions.
...
10. Delete dead code — unused functions, unreachable branches, never-called methods. Source
    control remembers everything.
</design_rules>

<concurrency_rules>
1. Keep concurrency-related code separate from other code — it has its own life cycle and is
   complex enough to warrant isolation (Single Responsibility Principle).
2. Severely limit the scope of shared data — minimize the number of places where shared data
   can be updated.
...
5. Keep synchronized sections as small as possible — locks are expensive and create contention.
6. Get nonthreaded code working first — test it independently with POJOs before introducing
   concurrency.
7. Treat spurious test failures as candidate threading issues — do not dismiss them as one-offs.
...
<examples>
<example>
<description>Bad naming and function design</description>
<bad_code>
...
<explanation>
The good version uses intention-revealing names: the function name says what it returns, the
loop variable says what it iterates over, and the condition is expressed as a readable method
call instead of a magic number comparison.
</explanation>
</example>

<example>
<description>Replacing a comment with a well-named function</description>
<bad_code>
...
<explanation>
The conditional logic is extracted into a method whose name communicates intent. The comment
becomes unnecessary.
</explanation>
</example>

<example>
<description>Command-query separation</description>
<bad_code>
...
<explanation>
The bad version conflates a query (does the attribute exist?) with a command (set the value).
The good version separates the two, making the code unambiguous.
</explanation>
</example>
</examples>

When generating, reviewing, or refactoring code, apply every applicable rule from the
sections above. Prioritize readability and simplicity. If you must choose between brevity and
clarity, choose clarity. Leave the code cleaner than you found it.
</system>